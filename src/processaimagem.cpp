...
String processa_imagem(uint8_t *rgb, int altura, int largura) {
    ...
    uint32_t ponto_direito_x = (uint32_t)(1 / 4. * largura);
    uint32_t ponto_direito_y = (uint32_t)(1 / 2. * altura);

    uint32_t ponto_superior_x = (uint32_t)(1 / 4. * largura);
    uint32_t ponto_superior_y = (uint32_t)(1 / 2. * altura);

    uint32_t ponto_inferior_x = (uint32_t)(1 / 4. * largura);
    uint32_t ponto_inferior_y = (uint32_t)(1 / 2. * altura);

    uint32_t ponto_central_x = (uint32_t)(1 / 4. * largura);
